...
이 엔진은 sid로 config를 읽어 공통 메커니즘(extract/formula/common/sklearn_model)에 위임만 한다.
  ...
  L2 model         : config.model.heuristic_fallback 면 common.model_predict, 아니면 sklearn_model.predict 직접
"""
...
class GenericEngine(ScenarioEngine):
    ...
    def model_predict(self, intent_id: str, features: dict[str, Any]) -> float:
        ...
        # 휴리스틱 폴백 없이 sklearn 직접 (cs: 충분한 학습 데이터)
